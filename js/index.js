...
// 		if (selectedMealType) {
// 			filteredRecipes = filteredRecipes.filter(recipe => recipe.mealType === selectedMealType);
// 		}
...
document.addEventListener("DOMContentLoaded", async () => {
		...
		filteredRecipes.forEach((recipe) => {
			...
			infoContainer.innerHTML = `
                ...
                <div class="recipe-meta">
                    ...
                    <span class="recipe-dish-type">${getDishTypeIcon(recipe.dishType)}</span>
                </div>
                <div class="recipe-buttons">
                    <button class="open-recipe" data-id="${recipe.id}">📖 Открыть</button>
                    <button class="delete-recipe" data-id="${recipe.id}">🗑 Удалить</button>
	...
	// Функция для получения иконки типа блюда
	function getDishTypeIcon(dishType) {
		if (dishType === "diet") {
			return "<span style='color: green;'>🍏 Диетическое</span>";
		} else {
			return "<span style='color: blue;'>🍔 Обычное</span>";
		}
	}
